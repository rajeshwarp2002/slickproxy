...
import (
	...
	"slickproxy/internal/ipallocator"
...
func isIPv6Address(host string) bool {
	return strings.Contains(host, ":") && net.ParseIP(host).To4() == nil
}
...
func resolveIPForRequest(rv request.Request) (string, net.IP, error) {
	...
	if rv.Credentials.IpMode == userdb.IPv6Mode && len(rv.Credentials.UserDetail.ProxyIPListv6) != 0 {
		...
		if rv.Credentials.Session != "" {
			ip, _ = ipallocator.RetrieveIPv6ForSessionKey(rv.Credentials.Session, rv.Credentials.Time, rv.Credentials.UserDetail)
			...
			ip, _ = ipallocator.RetrieveRandomIPv6Address(rv.Credentials.UserDetail.ProxyIPListv6)
		...
		if rv.Credentials.Session != "" {
			ip, _ = ipallocator.RetrieveIPv4ForSessionKey(rv.Credentials.Session, rv.Credentials.Time, rv.Credentials.UserDetail)
		} else if rv.Credentials.UserDetail.RotationIntervalSec == 0 || (rv.Credentials.UserDetail.LastIpTime == 0 || (rv.Credentials.UserDetail.LastIpTime+int64(rv.Credentials.UserDetail.RotationIntervalSec)) > epochTime) {
			ip, _ = ipallocator.RetrieveRandomIPv4Address(rv.Credentials.UserDetail.ProxyIPListv4)
